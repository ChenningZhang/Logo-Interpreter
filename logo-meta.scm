...
(define (handle-infix value line-obj env)
  (if (ask line-obj 'empty?)
	...
	(if (and (not (list? next-token)) (member? next-token '(+ - * / = < >)))
	    (handle-infix
	     (eval-prefix (make-line-obj (list (de-infix next-token) value (eval-prefix line-obj env))) env)
	     line-obj
	     env)
	    (begin (ask line-obj 'put-back next-token)
		   value)))))
...
(define (eval-definition line-obj)
  (define (get-formal-parameters)
	...
	(let ((next (ask line-obj 'next)))
	  (if (equal? next 'static)
	      '()
	  (if (variable? next)
	      (append (list (variable-name next)) (get-formal-parameters))
	      (get-formal-parameters))))))
  (define (get-static frame)
    (if (ask line-obj 'empty?)
	frame
  ...
  (let ((name (ask line-obj 'next))
	...
	(static (get-static (make-frame '() '()))))
    (set! the-procedures (cons (list name 'compound (length formals) (cons formals body) #f static)
			       the-procedures))
    '=no-value=))
  ...
  (cond ((null? exps) '=no-value=)
	((not (null? exps))
	 ...
	 (let ((value (eval-line (make-line-obj (car exps)) env)))
	   (cond ((eq? value '=stop=) '=stop=)
		 ((and (pair? value) (eq? (car value) '=output=)) value)
		 ((not (eq? value '=no-value=)) (error "You don't say what to do with " value))
		 (else (eval-sequence (cdr exps) env step)))))))

(define (step wd)
  (let ((defined-proc (lookup-procedure wd)))
    (if (compound-procedure? defined-proc)
	  (set-car! (step-flag defined-proc) #t))
    '=no-value=))

(define (unstep wd)
  (let ((defined-proc (lookup-procedure wd)))
    (if (compound-procedure? defined-proc)
	  (set-car! (step-flag defined-proc) #f))
    '=no-value=))
		      ...
		      (logo-apply proc
				  (cons env
					(collect-n-args (car (arg-count proc))
							line-obj
							env)) env))
		     ((and (< (arg-count proc) 0) (eq? paren-flag #f))
		      (logo-apply proc
				  (collect-n-args (abs (arg-count proc))
						  line-obj
						  env) env))
		     (else (logo-apply proc
...
(define (make-proc name arg para body)
  (list name 'compound arg (cons para body) #f))

(define (step-flag proc)
  (cddddr proc))
...
(define (make-frame variables values)
  (cons (cons '" TEST" variables) (cons '() values)))
